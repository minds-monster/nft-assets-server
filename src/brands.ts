...
export const getBrand = (slug: string) => BRANDS_BY_SLUG.get(slug) ?? null;

export const hasLiveCollection = (brand: any) => (brand?.collections?.length ?? 0) > 0;
 ...
 */
export const artRatio = (collection: any) =>
...
export const findCollection = (chain: string, address: string) => {
...
export const findCollectionByAddress = (address: string) => {
  const target = address?.toLowerCase();
